	...
	switch(action)
		...
		if("buy")
			...
			if(template.category == "Independent")
				...
				switch(choice)
					if("Пацифизм")
						if(tgui_alert_middle(usr, "Выбирая пацифиста вы отрекаетесь от военных действий и не имеете права заниматься мародёрством, но можете оказывать мед. помощь после завершения конфликта. Сделайте этот мир чуточку лучше!\n\n(Рекомендуется новичкам для изучения базовых возможностей и механик игры)", "Стиль игры", list("Подтвердить", "Отмена")) == "Подтвердить")
							template.prefix = "PISV"
						else
							return
					if("Нейтралитет")
						if(tgui_alert_middle(usr, "Вы выбираете путь независимого путника: будь то медик, исследователь, шахтёр или обычный турист. Будьте осторожны и помните возможности ходят рука об руку с опасностью. Удачного полёта.", "Стиль игры", list("Подтвердить", "Отмена")) != "Подтвердить")
							return
					if("Пиратство")
						if(tgui_alert_middle(usr, "Это история вашей смерти. Вы выбираете путь беззакония и всеобщей ненависти. Все вокруг будут пытаться вас убить и даже аванпост назначил за вашу голову награду. У вас нет друзей и надежды, но вас ведь это не волнует? Йо-хо-хо и бутылка рома!", "Стиль игры", list("Подтвердить", "Отмена")) == "Подтвердить")
