...
/obj/structure/deployable_barricade
	icon = 'mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi'
	anchored = TRUE
	density = TRUE
	climbable = TRUE
...
/obj/structure/deployable_barricade/proc/check_dir()
...
/obj/structure/deployable_barricade/CanAllowThrough(atom/movable/mover, turf/target)
	. = ..()
...
/obj/structure/deployable_barricade/update_overlays()
	...
	if(is_wired)
		if(!closed)
			. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "[barricade_type]_wire")
		else
			. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "[barricade_type]_closed_wire")
...
/obj/structure/deployable_barricade/attacked_by(mob/user, list/modifiers)
	. = ..()
	if(anchored)
		to_chat(usr, span_warning("It is secured to the floor, you can't turn it!"))
		return FALSE
...
/obj/structure/deployable_barricade/metal/update_overlays()
	...
	switch(barricade_upgrade_type)
		if(BARRICADE_TYPE_BOMB)
			. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "+explosive_upgrade_[damage_state]")
		if(BARRICADE_TYPE_MELEE)
			. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "+brute_upgrade_[damage_state]")
		if(BARRICADE_TYPE_ACID)
			. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "+burn_upgrade_[damage_state]")
...
/obj/structure/deployable_barricade/metal/proc/attempt_barricade_upgrade(obj/item/stack/sheet/metal/metal_sheets, mob/user, params)
	...
	var/static/list/cade_types = list(BARRICADE_TYPE_BOMB = image(icon = 'mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "explosive_obj"), BARRICADE_TYPE_MELEE = image(icon = 'mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "brute_obj"), BARRICADE_TYPE_ACID = image(icon = 'mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "burn_obj"))
		...
		for(var/direction in GLOB.cardinals)
			for(var/obj/structure/deployable_barricade/metal/plasteel/cade in get_step(src, direction))
				if(((dir & (NORTH|SOUTH) && get_dir(src, cade) & (EAST|WEST)) || (dir & (EAST|WEST) && get_dir(src, cade) & (NORTH|SOUTH))) && dir == cade.dir && cade.linked && cade.closed == closed)
					. += image('mod_celadon/_storge_icons/icons/items/obj1/barricade.dmi', icon_state = "[barricade_type]_[closed ? "closed" : "open"]_connection_[get_dir(src, cade)]")
...
/obj/item/storage/barricade/Initialize(mapload)
	...
	STR.max_items = 21
	STR.can_hold = typecacheof(list(/obj/item/quickdeploy/barricade, /obj/item/stack/sheet/mineral/sandbags, /obj/item/grenade/barrier))
