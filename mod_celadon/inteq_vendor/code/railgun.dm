...
/obj/structure/salvageable/railgun/examine_more(mob/user)
	. = ..()
		. += span_warning("The WAR’s discarding, sabot-jacketed two-stage projectile can pierce even the heaviest armor. \n\
							...
							Through a complex, delicate weave of heat exchangers, Heat sinks installed on the ship recycles the heat generated by a mech into usable energy. The system works well, but the delicate nature of the exchange renders it highly volatile. \n\
...
/obj/structure/salvageable/railgun_platform/crowbar_act(mob/living/user, obj/item/tool)
	if(prob(50)) //УБРАТЬ
		visible_message(span_danger("\The [src] internals scream, as the explosion enlights everyone around!"),//Scream прикольнее было будто или заменить на lights? Этоz
		...
		addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(explosion), get_turf(src), 0, 0, 3, 3), 1) // Not a normal explosion.
	else
		. = ..()
