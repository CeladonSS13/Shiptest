...
/datum/mission/ruin/fallen_montagne
	name = "проверка неясного сигнала"
...
/datum/mission/ruin/daughter
	...
	desc = "Нашу дочь несколько недель назад коммандировали на Фронтир. С того момента к нам не пришло не единой весточки от неё. Никто, даже СолГов, не знает что с ней... Пожалуйста! Найдите её и скажите, всё ли с ней хорошо..."
	value = 4000
	mission_limit = 1
...
/datum/mission/ruin/icw_documents
	name = "Доставка Боевых Планов Синдиката"
	desc = "Наша цель - сохранение истории Межкорпоративных Войн. Мы узнали об ещё одной крупице истории, которую требуется увековечить: Планы Боёв Синдиката. Наши источники сообщают, что планы находятся в пределах заданного объекта. Удостоверьтесь, там ли они, и доставьте их нам."
...
/datum/mission/ruin/nanotrasen_docs/generate_mission_details()
	...
	if(faction == /datum/faction/syndicate/cybersun)
		...
		desc = "Киберсан ВР крайне заинтересована в возвращении пакета документов, находящихся на объекте Нанойтрейзен, недавно атакованным кликой Рамзи. Доставьте пакет документов нам, а мы не поскупимся с вознаграждением."
...
/datum/mission/ruin/my_mech
	name = "Lucky #2"
	desc = "Слушай... Однажды, во время очередной межкорпоратской войны, меня занесло на мир-свалку. Наше подразделение было готово к высадке на фронт с минуты на минуту, однако приказа о высадке так и не последовало. Время шло, война уже подходила к концу, и по итоге мы с товарищами разлетелись по дома. Однако, спустя все эти годы, моя страсть к пилотированию мехов так и не угасла. Отправься на координаты, которые я тебе выдал, и проверь, на месте ли моя меха Lucky #2? Если она там, умоляю тебя, доставь её мне."
...
/datum/mission/ruin/dead_vanguard
	...
	desc = "Inteq потеряла связь с заказчиком и авангардистом, который выполнял его задание. На данный момент силы ЧВК находятся на заданиях более приоритетных, чем возвращение тела одного из сотрудников, а потому Inteq готова выдать это поручение любому, кто горазд вернуть тело авангардиста. Его имя - Kavur."
