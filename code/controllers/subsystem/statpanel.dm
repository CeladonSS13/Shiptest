...
#endif
	var/static/list/beta_notice = list("", "Если вы используете BYOND 516, некоторые интерфейсы и UI могут быть сломаны!", "Пожайлуста репортире о них на наш гитхаб, или переключитесь на BYOND 515")
...
/datum/controller/subsystem/statpanels/proc/return_object_images(client/load_from)
	...
	for(var/atom/turf_item as anything in obj_window.atoms_to_show)
		...
		obj_window.RegisterSignal(turf_item, COMSIG_QDELETING, /datum/object_window_info/proc/viewing_atom_deleted) // we reset cache if anything in it gets deleted
	return turf_items
