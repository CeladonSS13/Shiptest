...
/datum/controller/subsystem/shuttle/ui_act(action, params)
	...
	switch(action)
		if("select_template")
			if(S)
				...
				if(new_ship?.shuttle_port)
					...
					log_celadon_admin("\[SHUTTLE]: [key_name(user)] loaded [new_ship] ([S]) with the shuttle manipulator.</span>") // [CELADON_ADD] - logging admin actions.
		...
		if("blist")
			...
			log_celadon_admin("\[SHUTTLE]: [key_name_admin(user)] blacklisted [port_ship] from landing at [please_leave] with reason: [reason]") // [CELADON_ADD] - logging admin actions.
			return TRUE
