...
/datum/controller/subsystem/tgui/PreInit()
	...
	basehtml = replacetextEx(basehtml, "<!-- tgui:nt-copyright -->", "Nanotrasen (c) [CURRENT_SECTOR_YEAR]")
...
/datum/controller/subsystem/tgui/proc/try_update_ui(
		...
		datum/src_object,
		datum/tgui/ui)
	log_tgui("TGUI_DEBUG: try_update_ui called for [src_object] by [user?.ckey || "null"]")

	// Look up a UI if it wasn't passed
	if(isnull(ui))
		ui = get_open_ui(user, src_object)
		log_tgui("TGUI_DEBUG: get_open_ui returned [ui ? "found UI" : "null"]")

	// Couldn't find a UI.
	if(isnull(ui))
		log_tgui("TGUI_DEBUG: No UI found, returning null")
		return null

	ui.process_status()
	log_tgui("TGUI_DEBUG: UI status after process_status: [ui.status]")
	...
	if(ui.status <= UI_CLOSE)
		log_tgui("TGUI_DEBUG: UI status <= UI_CLOSE, closing UI")
		ui.close()
		return null

	log_tgui("TGUI_DEBUG: Sending UI update")
	ui.send_update()
	return ui
...
/datum/controller/subsystem/tgui/proc/on_open(datum/tgui/ui)
	log_tgui("TGUI_DEBUG: on_open called for [ui.src_object] by [ui.user?.ckey || "null"]")

	var/key = "[REF(ui.src_object)]"
	if(isnull(open_uis_by_src[key]) || !istype(open_uis_by_src[key], /list))
		open_uis_by_src[key] = list()
		log_tgui("TGUI_DEBUG: Created new UI list for key [key]")

	ui.user.tgui_open_uis |= ui
	var/list/uis = open_uis_by_src[key]
	uis |= ui
	open_uis |= ui
	log_tgui("TGUI_DEBUG: UI successfully added to tracking lists")
...
/datum/controller/subsystem/tgui/proc/on_close(datum/tgui/ui)
	log_tgui("TGUI_DEBUG: on_close called for [ui.src_object] by [ui.user?.ckey || "null"]")

	var/key = "[REF(ui.src_object)]"
	if(isnull(open_uis_by_src[key]) || !istype(open_uis_by_src[key], /list))
		log_tgui("TGUI_DEBUG: No UI list found for key [key], returning FALSE")
		return FALSE
	...
	if(length(uis) == 0)
		...
		log_tgui("TGUI_DEBUG: Removed empty UI list for key [key]")

	log_tgui("TGUI_DEBUG: UI successfully removed from tracking lists")
	return TRUE
