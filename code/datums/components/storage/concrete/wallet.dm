...
// /datum/component/storage/concrete/wallet/on_alt_click_async(datum/source, mob/user)	//  OLD
/datum/component/storage/concrete/wallet/open_storage(mob/user)	// [CELADON-ALERT] - ПКМ ПР 5241
	...
	if(istype(A) && A.front_id && !issilicon(user) && !(A.item_flags & IN_STORAGE)) //if it's a wallet in storage seeing the full inventory is more useful
		...
		// return
	// ..()	// OLD
		INVOKE_ASYNC(src, PROC_REF(attempt_put_in_hands), I, user)	// [CELADON-ALERT] - ПКМ ПР 5241
		return TRUE
	return ..()	// [/CELADON-ALERT]
*/ // [/CELADON-REMOVE]
