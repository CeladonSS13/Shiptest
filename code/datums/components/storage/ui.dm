...
/datum/component/storage/proc/orient2hud_legacy(mob/user, maxcolumns)
	...
	if(islist(numbered_contents))
		for(var/type in numbered_contents)
			...
			// CELADON EDIT START - FIXES_ICON_OUT_OF_BORDER - Убеждаемся, что координаты находятся в допустимых пределах
			...
			// CELADON EDIT START - Сбрасываем позицию элемента для предотвращения смещения
