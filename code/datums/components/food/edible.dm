...
/datum/component/edible/proc/check_liked(fraction, mob/eater)
	...
	if(!food_taste_reaction)
		...
		// [CELADON-ADD] - FIXES_PIZZABOX_AND_PIZZA - Плотоядные расы (Риолы и Таяраны) могут есть мясную еду, даже если она содержит нелюбимые ингредиенты
		else if((foodtypes & MEAT) && (human_eater.dna.species.liked_food & MEAT) && (human_eater.dna.species.disliked_food & VEGETABLES) && (human_eater.dna.species.disliked_food & GRAIN))
