...
/proc/log_econ(text)
	if (CONFIG_GET(flag/log_econ))
		WRITE_LOG(GLOB.world_economic_log, "ECON: [text]") // [CELADON-EDIT] - CELADON_COMPONENTS_LOGS - Делаем всей экономики вывод в файл economic.log // WRITE_LOG(GLOB.world_attack_log, "MONEY: [text]")
