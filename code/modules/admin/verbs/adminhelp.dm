...
/datum/admin_help_tickets/proc/browse_tickets(state)
	...
	for(var/I in l2b)
		...
		dat += "[span_adminnotice("<span class='adminhelp'>Ticket #[AH.id]")]: <A href='byond://?_src_=holder;[HrefToken()];ahelp=[REF(AH)];ahelp_action=ticket'>[AH.initiator_key_name]: [AH.name]</A></span><br>"
...
/datum/admin_help/New(msg, client/C, is_bwoink)
	...
	if(is_bwoink)
		...
		var/admin_number_present = send2tgs_adminless_only(initiator_ckey, "<@&1172909270442971207>, на сервере " + CONFIG_GET(string/servername) + " был создан новый Ticket #[id]: [msg]")
		log_admin_private("Ticket #[id]: [key_name(initiator)]: [name] - heard by [admin_number_present] non-AFK admins who have +BAN.")
		if(admin_number_present <= 0)
			to_chat(C, span_notice("В данный момент активные администраторы отсутствуют. Ваш тикет был передан администрации в Discord."), confidential = TRUE)
