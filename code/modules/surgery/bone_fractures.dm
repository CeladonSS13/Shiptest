...
/datum/surgery/repair_bone_compound/can_start(mob/living/user, mob/living/carbon/target) // [CELADON-EDIT] - FIXES_REPAIR_BONE_COMPOUND - исправляем неверное имя прока
	if(..())
		var/obj/item/bodypart/targeted_bodypart = target.get_bodypart(user.zone_selected)
		return(targeted_bodypart.get_wound_type(targetable_wound))
