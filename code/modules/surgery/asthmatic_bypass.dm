...
/datum/surgery/asthmatic_bypass/mechanical
	name = "Asthmatic Bypass (Mechanical)"
...
/datum/surgery_step/expand_windpipe/failure(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery, fail_prob)
	...
	if (iscarbon(target))
		...
		if (patient_chest)
			if (prob(30))
				wound_bonus += 60
			patient_chest.receive_damage(brute = 10, wound_bonus = wound_bonus, sharpness = SHARP_EDGED)
